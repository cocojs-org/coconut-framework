...
export function createTextInstance(text, hostContext) {
    if (__DEV__) {
...
export function createInstance(type, props, hostContext, internalInstanceHandle) {
    if (__DEV__) {
        const hostContextDev = hostContext;
...
export function commitMount(domElement, type, newProps) {
    switch (type) {
        case 'button':
        case 'input':
        case 'select':
