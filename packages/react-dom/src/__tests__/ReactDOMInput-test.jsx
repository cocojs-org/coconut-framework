...
describe('ReactDOMInput', () => {
    ...
    beforeEach(async () => {
        ...
        setUntrackedChecked = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'checked').set;
    ...
    it('should warn with checked and no onChange handler with readOnly specified', () => {
        cocoMvc.renderIntoContainer(<input type="checkbox" checked={false} readOnly={true} />, container);
        cocoMvc.unmountComponentAtNode(container);

        cocoMvc.renderIntoContainer(<input type="checkbox" checked={false} readOnly={false} />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'You provided a `checked` prop to a form field without an `onChange` handler. This will render a read-only field. If the field should be mutable use `defaultChecked`. Otherwise, set either `onChange` or `readOnly`.'
        );
    });
    ...
    it('updates the value on radio buttons from "" to 0', function () {
        cocoMvc.renderIntoContainer(<input type="radio" value="" onChange={function () {}} />, container);
        cocoMvc.renderIntoContainer(<input type="radio" value={0} onChange={function () {}} />, container);
    ...
    it('updates the value on checkboxes from "" to 0', function () {
        cocoMvc.renderIntoContainer(<input type="checkbox" value="" onChange={function () {}} />, container);
        cocoMvc.renderIntoContainer(<input type="checkbox" value={0} onChange={function () {}} />, container);
        expect(container.firstChild.value).toBe('0');
        expect(container.firstChild.getAttribute('value')).toBe('0');
    });
    ...
    it('should update `defaultValue` for uncontrolled input', () => {
        const node = cocoMvc.renderIntoContainer(<input type="text" defaultValue="0" />, container);
    ...
    it('should update `defaultValue` for uncontrolled date/time input', () => {
        const node = cocoMvc.renderIntoContainer(<input type="date" defaultValue="1980-01-01" />, container);
    ...
    it('should take `defaultValue` when changing to uncontrolled input', () => {
        const node = cocoMvc.renderIntoContainer(<input type="text" value="0" readOnly={true} />, container);
        expect(node.value).toBe('0');
        cocoMvc.renderIntoContainer(<input type="text" defaultValue="1" />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
    ...
    it('should throw for date inputs if `defaultValue` is an object where valueOf() throws', () => {
        ...
        const test = () =>
            cocoMvc.renderIntoContainer(<input defaultValue={new TemporalLike()} type="date" />, container);
    ...
    it('should throw for text inputs if `defaultValue` is an object where valueOf() throws', () => {
        ...
        const test = () =>
            cocoMvc.renderIntoContainer(<input defaultValue={new TemporalLike()} type="text" />, container);
        expect(test).toThrowError(new TypeError('prod message'));
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'Form field values (value, checked, defaultValue, or defaultChecked props)' +
                ' must be strings, not %s.' +
    ...
    it('should allow setting `value` to `true`', () => {
        ...
        stub = cocoMvc.renderIntoContainer(<input type="text" value={true} onChange={emptyFunction} />, container);
    ...
    it('should allow setting `value` to `false`', () => {
        ...
        stub = cocoMvc.renderIntoContainer(<input type="text" value={false} onChange={emptyFunction} />, container);
    ...
    it('should properly transition from an empty value to 0', function () {
        cocoMvc.renderIntoContainer(<input type="text" value="" onChange={emptyFunction} />, container);
        cocoMvc.renderIntoContainer(<input type="text" value={0} onChange={emptyFunction} />, container);
    ...
    it('should properly transition a text input from 0 to an empty 0.0', function () {
        cocoMvc.renderIntoContainer(<input type="text" value={0} onChange={emptyFunction} />, container);
        cocoMvc.renderIntoContainer(<input type="text" value="0.0" onChange={emptyFunction} />, container);
    ...
    it('should properly transition a number input from "" to 0', function () {
        cocoMvc.renderIntoContainer(<input type="number" value="" onChange={emptyFunction} />, container);
        cocoMvc.renderIntoContainer(<input type="number" value={0} onChange={emptyFunction} />, container);
    ...
    it('should remove the value attribute on submit inputs when value is updated to undefined', () => {
        ...
        cocoMvc.renderIntoContainer(<input type="submit" value={undefined} onChange={emptyFunction} />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'A component is changing a controlled input to be uncontrolled. ' +
                'This is likely caused by the value changing from a defined to ' +
                'undefined, which should not happen. ' +
    ...
    it('should remove the value attribute on reset inputs when value is updated to undefined', () => {
        ...
        // Not really relevant to this particular test, but changing to undefined
        // should nonetheless trigger a warning
        cocoMvc.renderIntoContainer(<input type="reset" value={undefined} onChange={emptyFunction} />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'A component is changing a controlled input to be uncontrolled. ' +
                'This is likely caused by the value changing from a defined to ' +
                'undefined, which should not happen. ' +
    ...
    it('should control radio buttons', () => {
        ...
        class RadioGroup {
            ...
            render() {
                return (
                    <div>
                        <input ref={this.aRef} type="radio" name="fruit" checked={true} onChange={emptyFunction} />
                        A
                        <input ref={this.bRef} type="radio" name="fruit" onChange={emptyFunction} />B
                        <form>
                            <input
                                ref={this.cRef}
                                type="radio"
            ...
            render() {
                ...
                return (
                    <div>
                        ...
                        <input type="radio" name={radioName} onChange={emptyFunction} checked={this.updated === true} />
                        <input
                            type="radio"
                            name={radioName}
                            onChange={emptyFunction}
    ...
    it('should warn with value and no onChange handler and readOnly specified', () => {
        cocoMvc.renderIntoContainer(<input type="text" value="zoink" readOnly={true} />, container);
        cocoMvc.unmountComponentAtNode(container);

        cocoMvc.renderIntoContainer(<input type="text" value="zoink" readOnly={false} />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
    ...
    it('should warn if value and defaultValue props are specified', () => {
        cocoMvc.renderIntoContainer(<input type="text" value="foo" defaultValue="bar" readOnly={true} />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
    ...
    it('should warn if controlled input switches to uncontrolled with defaultValue', () => {
        const stub = <input type="text" value="controlled" onChange={emptyFunction} />;
        cocoMvc.renderIntoContainer(stub, container);
        cocoMvc.renderIntoContainer(<input type="text" defaultValue="uncontrolled" />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'A component is changing a controlled input to be uncontrolled. ' +
                'This is likely caused by the value changing from a defined to ' +
                'undefined, which should not happen. ' +
    ...
    it('should warn if controlled checkbox switches to uncontrolled with defaultChecked', () => {
        const stub = <input type="checkbox" checked={true} onChange={emptyFunction} />;
        cocoMvc.renderIntoContainer(stub, container);
        cocoMvc.renderIntoContainer(<input type="checkbox" defaultChecked={true} />, container);
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'A component is changing a controlled input to be uncontrolled. ' +
                'This is likely caused by the value changing from a defined to ' +
                'undefined, which should not happen. ' +
    ...
    it('should not warn if radio value changes but never becomes controlled', () => {
        ...
        cocoMvc.renderIntoContainer(<input type="radio" />, container);
        cocoMvc.renderIntoContainer(<input type="radio" value="value" defaultChecked={true} />, container);
        cocoMvc.renderIntoContainer(<input type="radio" value="value" onChange={() => null} />, container);
        cocoMvc.renderIntoContainer(<input type="radio" />, container);
    });

    it('should not warn if radio value changes but never becomes uncontrolled', () => {
        cocoMvc.renderIntoContainer(<input type="radio" checked={false} onChange={() => null} />, container);
        cocoMvc.renderIntoContainer(
            <input type="radio" value="value" defaultChecked={true} checked={false} onChange={() => null} />,
            container
        );
    });
    ...
    it('sets type, step, min, max before value always', () => {
        ...
        cocoMvc.renderIntoContainer(
            <input value="0" onChange={() => {}} type="range" min="0" max="100" step="1" />,
            container
        );
    ...
    it('resets value of date/time input to fix bugs in iOS Safari', () => {
        ...
        jest.spyOn(document, 'createElement').mockImplementation(function (type) {
            const el = originalCreateElement.apply(this, arguments);
            const getDefaultValue = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'defaultValue').get;
            const setDefaultValue = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'defaultValue').set;
    ...
    describe('assigning the value attribute on controlled inputs', function () {
        ...
        it('does not set the value attribute on number inputs if focused', () => {
            const Input = getTestInput();
            application.start();
            const stub = cocoMvc.renderIntoContainer(<Input type="number" value="1" />, container);
            const node = cocoMvc.findDOMNode(stub);
        ...
        it('an uncontrolled number input will not update the value attribute on blur', () => {
            const node = cocoMvc.renderIntoContainer(<input type="number" defaultValue="1" />, container);
        ...
        it('an uncontrolled text input will not update the value attribute on blur', () => {
            const node = cocoMvc.renderIntoContainer(<input type="text" defaultValue="1" />, container);
    ...
    describe('setting a controlled input to undefined', () => {
        ...
        function renderInputWithStringThenWithUndefined() {
            ...
            class Input {
                ...
                render() {
                    return <input onChange={(e) => (this.value = e.target.value)} value={this.value} />;
                }
            }
    ...
    describe('When given a Symbol value', function () {
        it('treats initial Symbol value as an empty string', function () {
            cocoMvc.renderIntoContainer(<input value={Symbol('foobar')} onChange={() => {}} />, container);
            expect(consoleErrorSpy).toHaveBeenCalledWith(
