...
function defineMetadataId(
  ...
  if (descriptor) {
    if (descriptor.writable) {
      console.error(
        `框架会在类的上添加id属性，${MetadataClass.name} 原本的id属性会被覆盖`
      );
    } else {
      throw new Error(
        stringifyDiagnose(
          createDiagnose(DiagnoseCode.CO10015, MetadataClass.name)
        )
      );
    }
  }
...
function buildMetaClassIdMap(
  ...
  for (const cls of metaMetadataMap.keys()) {
    ...
    if (typeof id !== 'string' || !id) {
      throw new Error(
        stringifyDiagnose(createDiagnose(DiagnoseCode.CO10016, cls.name))
      );
    }
    if (idMetadataClassMap.has(id)) {
      throw new Error(
        stringifyDiagnose(
          createDiagnose(DiagnoseCode.CO10017, cls, idMetadataClassMap.get(id))
        )
      );
    }
