...
function validateHasTargetDecorator(
...
function validateNoNeedTargetDecorator(
  target: Class<Metadata>,
  metadatas: Metadata[] = []
) {
  const idx = metadatas.findIndex((metadata) => metadata instanceof Target);
    ...
    if (dupIdxs.length > 1) {
      diagnoseList.push(
        createDiagnose(
          ...
          target.name,
          className2DecoratorName(MetadataClass.name)
        )
      );
...
function isComponentDecorator(
  ...
  const MetadataClass = metadata.constructor;
  // 在一级复合中查找
  const classMetadatas = metaMetadataMap.get(
    MetadataClass as Class<Metadata>
  )?.classMetadata;
  ...
  for (const classMetadata of classMetadatas) {
    ...
    const parentClassMetadatas = metaMetadataMap.get(
      parentClass as Class<Metadata>
    )?.classMetadata;
...
function validateDecoratorTarget(
  ...
  for (let idx = 0; idx < metadatas.length; idx++) {
    const metadata = metadatas[idx];
    const classMetadatas = metaMetadataMap.get(
      metadata.constructor as Class<Metadata>
    )?.classMetadata;
    const targetMetadata: Target = classMetadatas?.find(
      (metadata) => metadata instanceof Target
    ) as Target;
...
function validateMethodComponentMustInConfiguration(
  beDecorated: { cls: Class<Metadata>; field: string; isConfigClass: boolean },
  ...
  for (let idx = 0; idx < metadataList.length; idx++) {
    ...
    if (metadata instanceof Component && !isConfigClass) {
      diagnoseList.push(
        createDiagnose(DiagnoseCode.CO10008, cls.name, field, cls.name)
      );
...
function validate(
  ...
  for (const metadataClass of keys) {
    ...
    const diagnose = validateHasTargetDecorator(
      metadataClass,
      value!.classMetadata
    );
    if (diagnose) {
  ...
  for (const [metadataClass, metadataList] of metaMetadataMap.entries()) {
    const diagnose = validateDuplicatedDecorator(
      metadataClass,
  ...
  // 业务类不需要添加@target装饰器
  for (const [metadataClass, metadataList] of bizMetadataMap.entries()) {
    const diagnose = validateNoNeedTargetDecorator(
      metadataClass,
      metadataList.classMetadata
    );
    if (diagnose) {
  ...
  for (const [metadataClass, metadataList] of bizMetadataMap.entries()) {
    ...
    for (const [
      field,
      methodMetadata,
    ] of metadataList.methodMetadata.entries()) {
      const diagnoses = validateDecoratorTarget(
