...
function buildComponentMetadataSet(metadataRepository: MetadataRepository) {
    ...
    for (const [beDecoratedCls, { classMetadata }] of metaMetadataMap.entries()) {
        const metadata: any = classMetadata.find((i) => i.constructor === Component);
        if (metadata) {
...
export { buildComponentMetadataSet, findComponentDecorator, findComponentDecoratorScope, clear };
