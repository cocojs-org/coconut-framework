...
class DotCocoBuilder {
    ...
    handleAddFile = (filePath: string) => {
        if (this.isTsTsxFile(filePath)) {
            ...
            const scanRlt = scanOneFile(this.project.genFullPath(filePath), match.decorator);
            if (scanRlt !== null) {
    ...
    watchProperties = () => {
        ...
        this.propertiesWatcher.on('add', () => mergeProperties(this.project, this.cmd));
        this.propertiesWatcher.on('change', () => mergeProperties(this.project, this.cmd));
        this.propertiesWatcher.on('unlink', () => mergeProperties(this.project, this.cmd));
    };
