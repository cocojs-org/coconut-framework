...
function merge(project: Project, cmd: string) {
    const defaultConfigStr = readFile(path.join(project.absPath, `properties/${defaultPropertiesName}`));
    const envConfigStr = readFile(path.join(project.absPath, `properties/${propertiesFileName(cmd)}`));
    let defaultConfig = {};
    try {
