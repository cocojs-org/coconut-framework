...
-   `coco-cli`: 命令行，基于webpack的应用开发、构建工具，以及基于 rollup 的组件库构建工具。是`npm包`。
-   `coco-ioc-container`: ioc容器，核心，在标准 es 装饰器规范之上封装了一套基于类和元数据的运行时框架。
-   `coco-mvc`: 框架入口，所有的运行时装饰器和组件都在这里导出。是`npm包`。
-   `coco-render`: 渲染组件。
-   `coco-router`: 客户端路由组件、装饰器。
...
-   `@cocojs/cli`和`@cocojs/mvc`都遵循[semver](https://semver.org/lang/zh-CN/)的规定。
-   `@cocojs/cli`和`@cocojs/mvc`2个包的版本号遵循主版本号一致，但次版本号、修订号不一致的规则，这样可以保证一定的灵活性，但又不会过于混乱。
